...
def visualize_data(t, Ts, mask, dem, results_dir):
    ...
    plt.xlabel("time (d)")
    plt.ylabel("T (C)")
    plt.savefig(make_sha_filename(results_dir / "breithorn_T.png"))
    plt.close()

    mask.plot()
    plt.savefig(make_sha_filename(results_dir / "breithorn_mask.png"))
    plt.close()

    dem.plot()
    plt.savefig(make_sha_filename(results_dir / "breithorn_dem.png"))
    plt.close()
...
def run_model_for_glacier(dem, mask, Ts, Ps, melt_factor, T_threshold, lapse_rate, z_weather_station, results_dir):
    ...
    plt.savefig(make_sha_filename(results_dir / "breithorn_net_balance_field.png"))
    plt.close()
...
def generate_output_table(zs, dt, Ts, Ps, melt_factor, T_threshold, lapse_rate, results_dir):
    ...
    output_csv_path = make_sha_filename(results_dir / "deltaT_impact.csv")
...
if __name__ == "__main__":
    ...
    data_dir = Path(__file__).resolve().parents[2] / "teaching_material/data/workshop-reproducible-research"
    weather_fl = data_dir / "own/weather.dat"
    mask_fl = data_dir / "own/mask_breithorngletscher/mask_breithorngletscher.asc"
    dem_fl = data_dir / "foreign/swisstopo_dhm200_cropped/dhm200_cropped.asc"
    ...
    # Visualize data
    visualize_data(t, Ts, mask, dem, results_dir)
